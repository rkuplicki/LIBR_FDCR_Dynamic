...
plot_one <- function(roi, this_label, idps, limits_spag, limits_y = c(-1, 1), prefix = 'dcr_', task = ''){
  ...
  #add 2.5 so time is blocks, uncentered
  p <- ggplot(plot_frame) + geom_hline(yintercept = 0, color = 'gray') + geom_line(aes_string(x = 'time + 2.5', y = mean_str, color = 'condition'), size = linesize) + 
    geom_errorbar(aes_string(x = 'time + 2.5', ymax = 'bar_top',                                                       ymin = 'bar_bottom', color = 'condition', width = 0.1), size = ebsize) +
                      theme(text = element_text(size=10)) + ggtitle(task) + ylab('Percent Signal Change') + xlab('Time (blocks)') + ylim(limits_y) 
...
plot_one_beh <- function(measure, this_label, idps, limits_spag, limits_y = c(1, 4), prefix = 'dcr_', task = ''){
  ...
  #swap condition levels back for this plot, so that drug is red and neutral is blue
  plot_frame$condition <- factor(plot_frame$condition, levels = c('drug', 'neutral'))
  p <- ggplot(plot_frame) + geom_line(aes_string(x = 'time + 2.5', y = mean_str, color = 'condition'), size = linesize) + geom_errorbar(aes_string(x = 'time + 2.5', ymax = 'bar_top',                                                       ymin = 'bar_bottom', color = 'condition', width = 0.1), size = ebsize) +
...
plot_combination <- function(roi, label, limits_spag = c(-2,2), limits_y = c(-1, 1), imaging = TRUE, forest_range = c(-0.5, 0.5)){
  ...
  for(this_list in list(tdcs_list, neurocaps_mcr_list, neurocaps_ocr_list, tacs_list)){
    ...
    plot_row <- data.frame(beta = c(condition_beta, conditionxtime_beta, tr_beta), se = c(condition_se, conditionxtime_se, tr_se),
                           effect = c('Drug>Neutral', 'Interaction', 'Time'), task = this_list$task)
  ...
  plot_frame$effect <- factor(plot_frame$effect, levels = c('Drug>Neutral', 'Time', 'Interaction'))
  ...
  #save the plot_frame, so we can easily plot a summary of old vs new analysis
  write.csv(plot_frame, paste0('plot_frame-', label, '.csv'), row.names = TRUE)
  ###
  ...
  ggsave(paste0(label, '-lineplot.png') , ggarrange(tdcs_list$p, neurocaps_mcr_list$p, neurocaps_ocr_list$p, tacs_list$p, ncol = 2, nrow = 2, 
                                                    common.legend = TRUE, legend = 'bottom'), width = pwidth, height = pheight)
...
png('corrplot_slopes_sxs.png', width = 1500, height = 1500)
corrplot.mixed(this_matrix, upper = 'ellipse', lower = 'number', tl.pos = 'lt', tl.cex = 1,#, lower.col = "black",
  upper.col = col2(50), lower.col = col2(50), p.mat = pvals_fdr, sig.level = 0.05)
dev.off()
